...
export default class SiteAccess extends React.Component<ISiteAccessProps, ISiteAccesState> {
        ...
        <div className={"row"}>
          ...
          {this.state.WSrequired ?
            (<div>
              ...
              <div style={{ marginTop: '10px' }} className={"row col-lg-12 col-md-12 col-sm-12"}>
                <span className={"col-lg-6 col-md-6 col-sm-12"}>SELECT WORKSTATION</span>
               ...
               <Toggle label="Enabled and checked" onText="On" offText="Off" checked={false} onChange={(e, val) => this.toggleChange(e, val, "WSrequired")} /> */}
                <div className={"row col-lg-6 col-md-6 col-sm-12"}>
                  <div className={"col-lg-8 col-md-8 col-sm-8"}>
                    <Dropdown disabled={this.state.workstationdisable} onChange={(e) => this.handleDropdownChange(e, "Workstationrequirestovisit")} placeholder="Select an option" defaultSelectedKey={[this.props.item.Workstationrequirestovisit]} options={this.dropDownValues["Workstationrequirestovisit"]} styles={dropdownStyles1} >
                  ...
                  <span style={{ padding: '8px', background: '#FBCE07', cursor: 'pointer' }}>View</span>
                    ...
                    <DefaultButton text="View Seat" onClick={this._handleClick} allowDisabledFocus />
                      </div>
                   {/* <div className={"col-lg-1 col-md-1 col-sm-1"} style={{ position: 'relative' }}>
            ...
            (<div>
              ...
              {this.state.MRrequired ?
                (<div> <div style={{ marginTop: '10px' }} className={"row col-lg-12 col-md-12 col-sm-12"}>
                  ...
                  <div style={{ marginTop: '10px' }} className={"row col-lg-12 col-md-12 col-sm-12"}>
                    <span className={"col-lg-6 col-md-6 col-sm-12"}>NAME OF ATTENDEES</span>
                    
                    <TextField disabled={this.state.mrDisabled} className={"col-lg-6 col-md-6 col-sm-12"} onChange={(value) => this._onTextChange(value, "NameOfAttend")} value={this.props.item.NameOfAttend} />
                    <div style={{display:'none'}}>
                      <TextField disabled={this.state.mrDisabled}  defaultValue={'0'} className={"col-lg-6 col-md-6 col-sm-12"} onChange={(value) => this._onTextChange(value, "Spacerequiredforvisit")} value={this.props.item.Spacerequiredforvisit} />
                    </div>
                  </div>
                  < div style={{ marginTop: '10px' }} className={"row col-lg-12 col-md-12 col-sm-12"}>
                    <span className={"col-lg-6 col-md-6 col-sm-12"}></span>
  ...
  public checkavailablity() {
    ...
    let ignoredates = [];
    let validDates = [];
    this.props.context.spHttpClient.get(this.props.context.pageContext.web.absoluteUrl + "/_api/web/lists/getbytitle('Site Access Request Form')/items?$filter=((Meeting_x0020_Room_x0020_StartT ge datetime'" + startdate + "') and (Meeting_x0020_Room_x0020_StartT le datetime'" + enddate + "') or (Meeting_x0020_Room_x0020_EndT ge datetime'" + startdate + "') and (Meeting_x0020_Room_x0020_EndT le datetime'" + enddate + "'))", SPHttpClient.configurations.v1).then(response => {
      ...
      response.json().then((result: any) => {
        ...
        this.props.context.spHttpClient.get(this.props.context.pageContext.web.absoluteUrl + "/_api/web/lists/getbytitle('Site Access Request Form')/items?$filter=((datetime'" + startdate + "' ge Meeting_x0020_Room_x0020_StartT) and (datetime'" + startdate + "' le Meeting_x0020_Room_x0020_EndT) or (datetime'" + enddate + "' ge Meeting_x0020_Room_x0020_StartT) and (datetime'" + enddate + "' le Meeting_x0020_Room_x0020_EndT))", SPHttpClient.configurations.v1).then(response2 => {
      ...
      response.json().then((result: any) => {
        ...
        this.props.context.spHttpClient.get(this.props.context.pageContext.web.absoluteUrl + "/_api/web/lists/getbytitle('Site Access Request Form')/items?$filter=((datetime'" + startdate + "' ge Daterequestingsiteaccessfrom) and (datetime'" + startdate + "' le DaterequestingsiteaccessTo) or (datetime'" + enddate + "' ge Daterequestingsiteaccessfrom) and (datetime'" + enddate + "' le DaterequestingsiteaccessTo))", SPHttpClient.configurations.v1).then(response2 => {
          response2.json().then((result2: any) => {
            ...
            this.refresh("workstationdisable");
            //this.setState({ selectedkey: "" });
            debugger;
          });
        });
        ...
        response.json().then((result: any) => {
          ...
          result.value.map((item) => {
            ...
            if (fieldName == "Workstationrequirestovisit") {
              let dropDown: any = { key: "", text: "" }
              dropDown.key = item.Title + "_" + item.Wing + "_" + item.Floor;
              dropDown.text = item.Title + "_" + item.Wing + "_" + item.Floor;
              this.dropDownValues[fieldName].push(dropDown);
              this.workstationRooms.push(item.Title+ "_" + item.Wing + "_" + item.Floor);
            }
            else {
              let dropDown: any = { key: "", text: "" }
